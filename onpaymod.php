<?php
/* -----------------------------------------------------------------------------------------
   $Id: onpay.php 998 2010/06/01 13:24:46 oleg_ossoft $

   os Shop - open source ecommerce solution
   http://osshop.ru
   http://osshop.com

   Copyright (c) 2010 os Soft Ltd.
   -----------------------------------------------------------------------------------------
   based on: 
...
// class methods
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_ONPAY_ZONE > 0) ) {
        $check_flag = false;
        $check_query = os_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_ONPAY_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function selection() {
      ...
      if (os_not_null($this->icon)) $icon = os_image($this->icon, $this->title);
...
if ($_SERVER["HTTP_X_FORWARDED_FOR"]) {
    ...
    function process_button() {
      global $customer_id, $order, $sendto, $osPrice, $currencies, $cart_onpay_id, $shipping;
      ...
      $process_button_string = os_draw_hidden_field('pay_mode', 'fix') .
                               os_draw_hidden_field('price', $order_sum) .
                               os_draw_hidden_field('currency', 'RUR') . 
                               os_draw_hidden_field('convert', 'yes') . 
                               os_draw_hidden_field('url_success', os_href_link(FILENAME_CHECKOUT_PROCESS, '', 'SSL')) . 
                               os_draw_hidden_field('url_fail', os_href_link(FILENAME_CHECKOUT_SHIPPING, '', 'SSL')) . 
                               os_draw_hidden_field('user_email', $order->customer['email_address']) . 
                               os_draw_hidden_field('md5', $hash) . 
                               os_draw_hidden_field('pay_for', substr($cart_onpay_id, strpos($cart_onpay_id, '-')+1))
                               ;
    ...
    function before_process() {
      global $customer_id, $order, $osPrice, $order_totals, $sendto, $billto, $languages_id, $payment, $currencies, $cart, $cart_onpay_id;
      global $$payment;
...
$osTemplate = new osTemplate;
  ...
  $it=0;
	...
	$osTemplate->assign('tpl_path', 'templates/'.CURRENT_TEMPLATE.'/');
	$osTemplate->assign('logo_path', HTTP_SERVER.DIR_WS_CATALOG.'templates/'.CURRENT_TEMPLATE.'/img/');
...
// unregister session variables used during checkout
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = os_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_ONPAY_STATUS'");
    ...
    function install() {
      ...
      os_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_ONPAY_STATUS', 'True', '6', '3', 'os_cfg_select_option(array(\'True\', \'False\'), ', now())");
      os_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_ONPAY_ALLOWED', '', '6', '4', now())");
      os_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_ONPAY_ID', '', '6', '5', now())");
      os_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_ONPAY_SORT_ORDER', '0', '6', '7', now())");
      os_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) values ('MODULE_PAYMENT_ONPAY_ZONE', '0', '6', '8', 'os_get_zone_class_title', 'os_cfg_pull_down_zone_classes(', now())");
      os_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_ONPAY_SECRET_KEY', '', '6', '9', now())");
      os_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_ONPAY_ORDER_STATUS_ID', '0', '6', '10', 'os_cfg_pull_down_order_statuses(', 'os_get_order_status_name', now())");
    }
